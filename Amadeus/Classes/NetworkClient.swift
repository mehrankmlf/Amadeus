...
final class NetworkClient : NetworkClientProtocol {
    ...
    init(session : URLSession = .shared,
         ...
         authorizationRequest : AuthorizationHttpRequest = AuthorizationHttpRequest()) {
            ...
            .tryMap { output, response -> Data in
                ...
                if httpResponse.statusCode == 401 {
                    self.authorizationRequest.authenticateRequest()
                        .sink { value in
                            if value {
                                self.request(with: request, decoder: decoder, scheduler: scheduler, responseObject: type)
